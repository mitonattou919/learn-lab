...
| 操作 | 方法 |
|------|------|
| コンテナを閉じてローカルに戻る | `Dev Containers: Reopen Folder Locally` |
...
| Node.js | `mcr.microsoft.com/devcontainers/typescript-node:20` |
...
# Python パッケージは requirements.txt 経由が一般的
# COPY requirements.txt /tmp/ && pip install -r /tmp/requirements.txt
```
...
> ⚠️ `postCreateCommand: "npm install"` は `package.json` が存在するプロジェクトで使う例です。
> 空のフォルダで試す場合はこの行を削除してください。
